...
ESVMSPATHFLOWFF_UP(I,J)       *make sure the ESVMSPATHFLOW(with VMS) proportion of the corresponding OD(ij) reflect the situation when there is no workzone
ESVMSPATHFLOWFF_LO(I,J)
...
PATH_PROF_UP(I,J,P)$PATH_PRO(I,J,P)..ESPATHFLOW(I,J,P)/DEMAND(I,J)=L=PATH_PRO(I,J,P)*1.1;
PATH_PROF_LO(I,J,P)$PATH_PRO(I,J,P)..ESPATHFLOW(I,J,P)/DEMAND(I,J)=G=PATH_PRO(I,J,P)*0.9;
ESVMSPATHFLOWF(I,J,P,K)$(VMS_PATH_LINK(I,J,P,K))..ESVMSPATHFLOW(I,J,P,K)=E=ESPATHFLOW(I,J,P);
ESVMSPATHFLOWFF_UP(I,J)$(DEMAND(I,J))..SUM((P,K)$(VMS_PATH_LINK(I,J,P,K) AND VMS(K) AND DEMAND(I,J)),ESVMSPATHFLOW(I,J,P,K)/(DEMAND(I,J)+0.0001))=L=0.50*1.1;
ESVMSPATHFLOWFF_LO(I,J)$(DEMAND(I,J))..SUM((P,K)$(VMS_PATH_LINK(I,J,P,K) AND VMS(K) AND DEMAND(I,J)),ESVMSPATHFLOW(I,J,P,K)/(DEMAND(I,J)+0.0001))=G=0.50*0.9;
...
SOF..CONSTANT=E=12;
SOFF(I,J)$DEMAND(I,J)..SO(I,J)=E=SUM(P,PATH_TT(I,J,P)*ESPATHFLOW(I,J,P));


MODEL ODE /ALL/;
SOLVE ODE USING NLP MINIMIZING CONSTANT;
